...
**Live:** https://techshield-tech.github.io/color-converter/

Part of [MMOALL Developer Tools](https://mmoall.com/tools).
Also available at [mmoall.com/tools/color-converter](https://mmoall.com/tools/color-converter).
...
- [`@mmoall/tool-kit`](https://github.com/techshield-tech/tool-kit) for shared shell/UI/theme/embed/SEO code, plus React
...
Shared shell/UI/theme/embed/SEO code (header/footer, theme handling, embed mode, UI
primitives, SEO tags) is no longer a local `src/shell/` directory — it now comes from
the `@mmoall/tool-kit` npm package (see `main.tsx` and `vite.config.ts`).
...
```bash
git clone https://github.com/techshield-tech/color-converter.git
cd color-converter
bun install
bun dev
```

Open the URL Vite prints — by default **http://localhost:5173/color-converter/**
(note the `/color-converter/` path, see [Base path](#base-path)).
...
`@mmoall/tool-kit/vite`, called in `vite.config.ts`):
...
| otherwise (default)     | `/color-converter/` | GitHub Pages                |
...
1. Go to [vercel.com/new](https://vercel.com/new) and import the
   `techshield-tech/color-converter` repository.
...
`bun run build` with the default `/color-converter/` base, and publishes `dist/`.
...
<iframe
  id="color-converter"
  src="https://techshield-tech.github.io/color-converter/?embed=1&theme=dark"
  style="width: 100%; border: 0;"
  title="JSON Formatter"
></iframe>

<script>
  const iframe = document.getElementById('color-converter');

  window.addEventListener('message', (event) => {
    const data = event.data;
    if (data?.slug !== 'color-converter') return;
...
| iframe → parent | `{ type: 'mmoall-tool:ready', slug: 'color-converter' }`          | Posted once on mount (embed mode only) |
| iframe → parent | `{ type: 'mmoall-tool:height', slug: 'color-converter', height }` | Posted whenever the document height changes (embed mode only) |
